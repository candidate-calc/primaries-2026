# -*- coding: utf-8 -*-
# מייצר docs/beta/data.js לגרסת הביתא: אותה הרכבת נתוני מועמדים כמו build_site_data,
# בתוספת נתוני המלצות (endorsements) ושכבות tier לברירות המחדל.
...
# חותמת "עודכן לאחרונה" — מחושבת אוטומטית מזמן ריצת הסקריפט. גרסת הביתא אינה נוגעת ב-README.md.
now = datetime.now()
...
with open(os.path.join(BETA,'data.js'),'w',encoding='utf-8') as f:
    f.write('window.PRIMARIES_DATA = ')
    json.dump(data, f, ensure_ascii=False)
    f.write(';')

print('beta data.js written,', len(cands), 'candidates,', len(dossiers), 'dossiers')
print('endorse edges: raw', len(raw_edges), '-> deduped', len(endorse))
print('endorseRef', endorseRef)
